  ...
  <div class="full-height">
    ...
    <v-row v-if="data.length > 0">
      <v-col cols="12" sm="4" v-for="i in 3" :key="i">
        <player-card :class="{[`place-${i}`]: true}" :uuid="data[i - 1].uuid" :name="data[i - 1].name"> 
...
export default class Leaderboard extends Vue {
  ...
  async fetchLeaderboard(): Promise<void> {
    ...
    this.data = await fetch(`https://api.lergin.de/hive/leaderboard/${this.id}?limit=100`).then(res => res.json());
  }
