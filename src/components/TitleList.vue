  ...
  <div>
    ...
    <v-list two-line subheader v-if="!loading">
      <template v-for="group of titles">
        ...
        <v-subheader
          class="subtitle-2"
          :key="`${group.group}-subheader`"
          style="text-transform: capitalize;"
          >{{ group.group | formatGroupName }}</v-subheader
        >
        <v-list-item v-for="title of group.titles" :key="title.id">
          <v-list-item-content>
            <v-list-item-title
              ><mc-format :text="title.mcFormatName"></mc-format
            ></v-list-item-title>
            <v-list-item-subtitle v-if="!title.topRank"
              >Starting at
              <span class="subtitle-2">{{
                title.requiredPoints | toLocaleString
              }}</span>
              points</v-list-item-subtitle
            >
            <v-list-item-subtitle v-if="title.topRank"
              >Top Rank</v-list-item-subtitle
            >
          </v-list-item-content>
        </v-list-item>
      </template>
...
export default class TitleList extends Vue {
  ...
  async loadTitles() {
    this.loading = true;
    const titles = await ((GameTypes as any) as { [key: string]: GameType })[
      this.gameType
    ].titles();

    const groupMap = titles.reduce(
      (acc, title) => {
        if (title.topRank) {
          acc["Top Rank"] = [title];
        } else if (title.group) {
