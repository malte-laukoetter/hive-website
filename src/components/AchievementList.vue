...
<template>
  <v-card>
    <v-card-title>
      <span>{{title}}</span>
      <span class="subtitle-1 ml-2">{{achievements.filter(a => a.unlocked).length}} / {{achievements.length}}</span>
    </v-card-title>
    <v-list three-line subheader>
      <template v-for="achievement of achievements">
        <achievement-list-item :key="achievement.id" :achievement="achievement"></achievement-list-item>
      </template>
    </v-list>
  </v-card>
</template>
