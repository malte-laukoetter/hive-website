...
export default class ServerUniquePlayerLineChart extends Vue {
  ...
  async fetchData(): Promise<void> {
    const dataObject: {[key: string]: number} = await fetch('https://api.lergin.de/hive/uniqueplayers').then(res => res.json())
    const data = Object.entries(dataObject)

    this.data = data.map(([key, value]) => ({x: parseInt(key, 10), y: value}))
